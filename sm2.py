...
def process_iterate(cqt_data, sr, n_samples, midi_it, n):
	...
	duration_per_segment = n_samples * 1.0 / (cqt_data.shape[1] * sr)
	...
	for i in range(m):
		...
		for interval in midi_it.search(begin, end):
			if predicate_note_overlaps(begin, end, interval.begin, interval.end):
				...
				j += 1
			k += 1
		print('BEGIN {}, END {} had {}/{} notes'.format(begin, end, j, k))
		t_1 = time.time()
		# grab the CQT segments
		ys[i] = np.absolute(cqt_data[:, i:i + n])
		t_2 = time.time()
		t_midi += t_1 - t_0
		t_cqt += t_2 - t_1
		if (i + 1) % 100 == 0:
...
def process(p, sfs):
	...
	t1 = time.time()
	print('MIDI took {:.3f} s.'.format(t1 - t0))
